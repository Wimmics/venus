...
export function getEncodingAddPicker(component, encoding = {}) {
	...
	if (component === "venus-sankey") {
		return {
			scopes: [
				...visualizationRootProperties(),
				markScope("nodes", "Nodes", "nodes", { size: false }, [
					singleProperty("fields", "Stage fields", SNIPPETS.sankeyNodeFields),
					singleProperty("label", "Label", SNIPPETS.constantLabel)
				]),
				markScope("links", "Links", "links", {}, [
					singleProperty("value", "Flow value", SNIPPETS.sankeyLinkValue),
					singleProperty("opacity", "Opacity", SNIPPETS.linkOpacity)
				]),
				{
					id: "sort",
					label: "Sort",
					properties: [
						singleProperty("by", "Sort by", SNIPPETS.sankeySort),
						singleProperty("order", "Order", SNIPPETS.sortOrder)
					]
				}
			]
		};
