...
export async function buildScatterPlot({
  endpoint = null,
  query = null,
  jsonData = null,
  proxyUrl = null,
  ...
  const mapper = createSparqlMapper(VIS_TYPES.VENUS_SCATTERPLOT);
  ...
  return buildVis({
    ...
    mapToVis: (raw, { mapper: mapperInstance, encoding: mapping, encodingManager: manager }, log) => {
      ...
      log?.debug?.("scatter-plot mapped", { rowCount: chart.rows?.length || 0 });
      return { chart, meta };
    }
  });
}
