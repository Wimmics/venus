export class SizeRangeCalculator {
	...
	createSizeRange({ data, field, scaleType = "linear", range = null, label = "Size" } = {}) {
		...
		if (!Array.isArray(range) || range.length === 0) {
			console.warn(`No size range provided (${label}). Using default range [${fallbackRange.join(", ")}].`);
		...
		if (isQuantitative) {
			...
			console.warn(`Invalid quantitative size range (${label}): [${range.join(", ")}]. Using default range [${fallbackRange.join(", ")}].`);
			return fallbackRange;
		}
		...
		console.warn(`Invalid ordinal size range (${label}): [${range.join(", ")}]. Using default range [${fallbackRange.join(", ")}].`);
		return fallbackRange;
	}
