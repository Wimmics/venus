...
export class SparqlToBarChartMapper extends SparqlToCartesianMapper {
	...
	_buildCanonicalChart(rows, encoding) {
		...
		const labelsConfig = encoding?.bars?.labels;
	...
	_buildSimpleBars({ xValues, aggregated, xField, yField, labelsConfig }) {
		return xValues.map((xCategory) => {
			...
			return {
				x: xCategory,
				value,
				label: this._resolveLabelFromDatum({ labelsConfig, datum, fallbackField: xField }),
				datum
			};
		});
	}
	
	_buildGroupedBars({ xValues, subCategories, aggregated, xField, yField, splitField, labelsConfig }) {
