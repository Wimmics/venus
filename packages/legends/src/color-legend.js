...
export class ColorLegend extends HTMLElement {
  ...
  render() {
    ...
    const items = domain.slice(0, 10).map((value, i) => {
      // Use D3 scale if available for accurate color mapping, otherwise use range array
      let color = '#999';
    ...
    const moreInfo = domain.length > 10 
      ? `<div class="more-info">+${domain.length - 10} more</div>` 
      : '';
