...
| `bars` | Configures bar mark appearance. Use `bars.color` and `bars.size` for mark-level mappings. | - | ✗ |
| `axis` | Axis options are configured inside `x.axis` and `y.axis` (titles, label angle/offset, tick formatting). | [`axis`](../encoding/types/axis.md) | ✗ |
...
| `groups` | Defines the field used to split categories into grouped/stacked series. | [`groups`](../encoding/cartesian/groups.md) | ✗ |
| `stack` | Chooses stacking mode (`false`, `true`, `"normalize"`). | [`stack`](../encoding/cartesian/stack.md) | ✗ |
